...
int main()
{
    Store<int>s1,s2; //定义Store<int>类对象，其中数据成员item为int类型
    s1.putElem(3);  //向对象s1中存入数据（初始化对象s1）
    s2.putElem(-7); //向对象s1中存入数据（初始化对象s1）
    cout <<s1.getElem()<< " "<<s2.getElem() << endl; //输出对象s1和s2的数据成员

    Student g={1000,23};  //定义Studuent类型结构体变量的同时赋予初值
    Store<Student>s3;  //定义store<Student>类对象s3，其中数据成员item为Student类型
    s3.putElem(g);  //向对象s3中存入数据（初实化对象s3）
    ...
    //由于d未经初始化，在执行函数的d.getElement（）过程中导致程序终止
