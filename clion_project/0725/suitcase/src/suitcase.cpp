...
void imageCallback ( const sensor_msgs::ImageConstPtr& msg )
{
	...
	try
	{
        ...
        ROS_ERROR_NAMED ( "suitcase","img_error" );
	}
...
{
    ROS_ERROR_NAMED ( "suitcase","SuitcaseThreadOn" );
...
//	ret.state=2;
	...
	{
		//get image
        ROS_ERROR_NAMED ( "suitcase","ThreadOn" );
	    if ( ( !reciveImageFlag ) )
		{
			continue;
		}
	    if(!img.data)  {ROS_ERROR_NAMED ( "suitcase","!img.data" );  continue;}
	    if(img.empty()){ROS_ERROR_NAMED ( "suitcase","img.empty()" ); continue;}

		m1.lock();
		img.copyTo ( frame );
		m1.unlock();
        ROS_ERROR_NAMED ( "suitcase","frame" );
		FrameId++;
//#ifdef DEBUG
...
//#endif
		...
		{
            ROS_ERROR_NAMED ( "suitcase","start detect" );
			BoxInfo binfo= {0};

			if ( FrameId%3==0 )
			{
			    if ( FrameI==10002 )FrameI=0;
				suitdetector.detect ( frame,res );
				if(res.empty())
				{
					// 如果 map 为空 doSomething
                    ROS_ERROR_NAMED ( "suitcase","res.empty()" );
					continue;
				}
			}
			else
			...
			for ( iter=res.begin(); iter!=res.end(); iter++ )
			{
				...
				if ( suitcase==1 )
				{
					...
					{
                        ROS_ERROR_NAMED ( "suitcase","suitcase==1" );
						float varscore=itr->second;
						float iouscore=0.00;
						Rect rt=itr->first;
						if (!rt.area()) break;
						if(varscore<=0.8) break;
			...
			if (0<binfo.xc-binfo.w/2 && 0<binfo.yc -binfo.h/2 &&binfo.w>0 && binfo.h>0 &&binfo.xc+binfo.w/2<=frame.cols && binfo.yc+ binfo.h/2<=frame.rows)
			{
...
//			cout<<"1111"<<endl;
            ROS_ERROR_NAMED ( "suitcase","start track" );
			...
			cv::Rect rect = cv::Rect(currentrackingpos.x,currentrackingpos.y,currentrackingpos.width,currentrackingpos.height);
			rect &= cv::Rect(0,0,frame.cols,frame.rows);
//			cout<<"222"<<endl;
            ROS_ERROR_NAMED ( "suitcase","22222" );
			cv::rectangle(frame,rect,cv::Scalar ( 0,0,255 ) );
			cv::imshow ( "tracking",frame );
			cv::waitKey ( 10 );
#endif

		 if(Trackingscore-0.5<0.000001)
            {   //tracking loss
		  ...
		  {
             if (suittracking._framecout==10000) suittracking._framecout=0;
...
int main ( int argc, char** argv )
{
	...
	SuitcaseCarryoverOpen=n.subscribe ( "/carryover_open", 10, &carryover_callback,ros::TransportHints().tcpNoDelay ( true ) );
	image_transport::Subscriber imgesub = it.subscribe ( "image_raw2", 10, imageCallback );

	suitcase_pub_resbon = n.advertise<sql_node::custom_return> ( "/custom_return", 10 );
	chatter_pub_receive= n.advertise<sql_node::custom_return>("custom_return", 10);
	suitcase_pub_Carryover = n.advertise<sql_node::imageinfo> ( "/suitcase_carryover_warning", 10);
	while(chatter_pub_receive.getNumSubscribers()<=0)
	{
	}
	sql_node::custom_return topen;
	 topen.id="";
	 topen.topicName="suitcase_open";
	 topen.state=2;
	 topen.param="true";
	 chatter_pub_receive.publish(topen);
