...
//#endif
		...
		if ( suittracking.staus==idle )
		{
			...
			if (0<binfo.xc && 0<binfo.yc  &&binfo.w>0 && binfo.h>0 &&binfo.xc+ binfo.w/2<=frame.cols && binfo.yc+ binfo.h/2<=frame.rows)
			{
...
//cout<<binfo.xc<<binfo.w/2<<binfo.yc<<binfo.h/2<<endl;
		...
		else
		{
			...
			cv::imshow ( "tracking",frame );
			cv::waitKey ( 10 );
			if(!frame.empty()) cv::destroyWindow("tracking");
			if(!frame.data) cv::destroyWindow("tracking");
